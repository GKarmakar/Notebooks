...
"""
def get_music(pred, indices_tones):
    
    # this assumes that pred is a list of indices
    predicted_tones = [indices_tones[p] for p in pred]
    
    predicted_tones = np.array(predicted_tones).replace(' A',' C').replace(' X',' C')
    ...
    print('After pruning, generated %s sounds using the current chords' % (len([k for k in sounds if isinstance(k, note.Note)])))
...
def generate_music(inference_model, corpus = corpus, abstract_grammars = abstract_grammars, tones = tones, tones_indices = tones_indices, indices_tones = indices_tones, T_y = 100, max_tries = 1000, diversity = 0.5):
    ...
    num_chords = len(chords)                              # number of different set of chords
    ...
    for i in range(1, num_chords):
        ...
        _, predicted_tones = predict_and_sample(inference_model, Ty = Ty)
...
def generate_music(inference_model, corpus = corpus, abstract_grammars = abstract_grammars, tones = tones, tones_indices = tones_indices, indices_tones = indices_tones, T_y = 100, max_tries = 1000, diversity = 0.5):
    ...
    for i in range(1, num_chords):
        ...
        # Generate a sequence of tones using the model
        _, indices = predict_and_sample(inference_model, Ty = 100)
